...
class BEBOCS_API(object):
    def __init__(self, args, device, dataset, model):
        ...
        # 参数1
        self.args.client_num_in_total = global_client_num_in_total # 本地训练客户端总数
        # self.args.client_num_per_round = global_client_num_per_round # 本地训练每轮客户端数量
        self.client_list = []
        self.client_train_prob = [] # 客户训练概成功率列表
        self.client_virtual_queue = [] # 客户虚拟队列
        self.client_sus= [] # 客户端训练成功次数
        self.client_fal= [] # 客户端训练失败次数
    ...
    def _setup_clients(
        ...
        for client_idx in range(self.args.client_num_in_total):
            ...
            self.client_train_prob.append(0.5) # 客户训练概成功率列表
            self.client_virtual_queue.append(0) # 客户虚拟队列
            self.client_sus.append(0) # 客户端训练成功次数
            self.client_fal.append(0) # 客户端训练失败次数
        logging.info("############setup_clients (END)#############")

    def train(self):
        global client
        client_selected_times = [0 for i in range(self.args.client_num_in_total)]
        logging.info("self.model_trainer = {}".format(self.model_trainer))
        ...
        for round_idx in range(self.args.comm_round):
            ...
            logging.info("client_train_prob = %s" % str(self.client_train_prob))
            for idx, client in enumerate(self.client_list):
                ...
                if client.client_idx in client_indexes:
                    ...
                    if self.judge_model(self.client_train_prob[client.client_idx]) == 1: # 判断是否成功返回模型
                        ...
                        self.client_sus[idx] += 1
                        logging.info("client: " + str(client.client_idx)+" successfully return model")
                    else:
                        self.client_fal[idx] += 1


            # 统计客户每个人的被选择次数、更新被选中用户返回模型概率估计以及更新全部客户虚拟队列
            for i, client in enumerate(self.client_list):
                if client.client_idx in client_indexes:
                    client_selected_times[i] += 1
                    # 更新被选中客户返回模型概率估计
                    intg1=integrate.quad(lambda u:(u**(self.client_sus[i]+1))*((1-u)**(self.client_fal[i]+1)),0,1)[0]
                    intg2=integrate.quad(lambda r:r**(self.client_sus[i]+2)*((1-r)**(self.client_fal[i]+1)),0,1)[0]
                    self.client_train_prob[i] = float(intg2/intg1)
                # 更新全部客户虚拟队列
                gam=self.getGamma()
                x=1 if i in client_indexes else 0
                self.client_virtual_queue[i] = max(self.client_virtual_queue[i]+gam-x,0)
            ...
            wb.save(self.args.excel_save_path + self.args.model + "_[" + self.args.dataset +"]_bebocs_training_results_NIID"+ str(self.args.experiment_niid_level) +".xlsx")
    ...
    def getGamma(self):
        return min(self.client_list, key=lambda x: x.local_sample_number).local_sample_number/self.train_data_num_in_total

    def judge_model(self,prob): # 基于随机数结合概率判断是否返回模型
    ...
    def _client_sampling(self):
        ...
        if client_num_in_total == client_num_per_round:
            ...
            client_score=[] # 计算所有待选择客户端的分数
            for idx, client in enumerate(self.client_list):
                client_score.append(client.get_sample_number()*self.client_train_prob[idx]+self.client_virtual_queue[idx])
            sorted_idx = np.argsort(client_score) # 按分数排序的客户端下标
            client_indexes = sorted_idx[-1*num_clients:] # 选出分数最高的num_clients个客户端
...
def plot_accuracy_and_loss(self, round_idx):
    ...
    plt.clf()
    plt.suptitle("Bebocs" + "_[" + self.args.dataset +"]_NIID"+ str(self.args.experiment_niid_level))
