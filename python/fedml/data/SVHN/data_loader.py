...
def read_data_distribution(
    filename="./data_preprocessing/non-iid-distribution/SVHN/distribution.txt",
):
    distribution = {}
...
def read_net_dataidx_map(
    filename="./data_preprocessing/non-iid-distribution/SVHN/net_dataidx_map.txt",
):
    net_dataidx_map = {}
    with open(filename, "r") as data:
        for x in data.readlines():
...
def load_SVHN_data(datadir):
    ...
    cifar10_train_ds = SVHN_truncated(
        datadir, split="train", download=True, transform=train_transform
    )
    cifar10_test_ds = SVHN_truncated(
        datadir, split="test", download=True, transform=test_transform
    )
...
def partition_data(dataset, datadir, partition, n_nets, alpha):
    ...
    if partition == "homo":
        ...
        dataidx_map_file_path = (
            "./data_preprocessing/non-iid-distribution/SVHN/net_dataidx_map.txt"
        )
        net_dataidx_map = read_net_dataidx_map(dataidx_map_file_path)

    if partition == "hetero-fix":
        distribution_file_path = (
            "./data_preprocessing/non-iid-distribution/SVHN/distribution.txt"
        )
...
def get_dataloader_test(
    ...
    return get_dataloader_test_SVHN(
...
def get_dataloader_SVHN(datadir, train_bs, test_bs, dataidxs=None):
    ...
    train_ds = dl_obj(
        datadir, dataidxs=dataidxs, split="train", transform=transform_train, download=True
    )
    test_ds = dl_obj(datadir, split="train", transform=transform_test, download=True)
