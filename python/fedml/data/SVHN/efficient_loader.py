...
from .without_reload import SVHN_truncated, SVHN_truncated_WO_reload
...
def load_svhn_data(datadir, process_id, synthetic_data_url, private_local_data, resize=32, augmentation=True, data_efficient_load=False):
    train_transform, test_transform = _data_transforms_svhn()

    is_download = True

    if data_efficient_load:
        svhn_train_ds = SVHN(datadir, split='train', download=True, transform=train_transform)
        svhn_test_ds = SVHN(datadir, split='test', download=True, transform=test_transform)
    else:
        svhn_train_ds = SVHN_truncated(datadir, split='train', download=is_download, transform=train_transform)
        svhn_test_ds = SVHN_truncated(datadir, split='test', download=is_download, transform=test_transform)
...
def partition_data(dataset, datadir, partition, n_nets, alpha, process_id, synthetic_data_url, private_local_data):
    ...
    X_train, y_train, X_test, y_test, cifar10_train_ds, cifar10_test_ds = load_svhn_data(datadir, process_id, synthetic_data_url, private_local_data)
...
def get_dataloader_SVHN(
    ...
    if data_efficient_load:
        ...
        train_ds = dl_obj(
            datadir, dataidxs=dataidxs,transform=transform_train, full_dataset=full_train_dataset
        )
        test_ds = dl_obj(datadir,transform=transform_test, full_dataset=full_test_dataset)
    else:
...
def load_partition_data_distributed_svhn(
    process_id,
    dataset,
    data_dir,
    partition_method,
...
def efficient_load_partition_data_svhn(
    args,
    dataset,
    data_dir,
    partition_method,
